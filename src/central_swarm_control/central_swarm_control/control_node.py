...
class SwarmConsensusController(Node):
    def __init__(self):
        ...
        # Per-drone crash detection for clean CSV logging.
        # Once a drone's Kalman filter diverges after a physical crash,
        # we freeze its logged position at the last known good value
        # and log NaN for dynamic channels (vel, attitude, gyro).
        self.drone_crashed = [False] * self.num_drones
        self.last_good_positions = np.zeros((self.num_drones, 3))
    ...
    def control_loop(self):
        ...
        # Log row — with per-drone crash detection
        nan = float('nan')
        row = [round(t, 3), self.state, self.fault_type, round(self.current_motor_health, 3)]
        for i in range(self.num_drones):
            px = float(self.current_positions[i][0])
            py = float(self.current_positions[i][1])
            pz = float(self.current_positions[i][2])

            # Detect crash: position outside reasonable flight envelope
            if not self.drone_crashed[i]:
                if abs(px) > 2.0 or abs(py) > 2.0 or pz > 1.5 or pz < -0.1:
                    self.drone_crashed[i] = True
                    self.get_logger().warn(
                        f"🔻 [{self.drones[i]}] Crash detected at t={round(t,2)}s — "
                        f"pos=({px:.2f},{py:.2f},{pz:.2f}). Freezing logged position."
                    )
                else:
                    self.last_good_positions[i] = [px, py, pz]

            if self.drone_crashed[i]:
                # Log last known good position; NaN for dynamic channels
                gp = self.last_good_positions[i]
                row.extend([
                    float(gp[0]), float(gp[1]), float(gp[2]),
                    nan, nan, nan,   # velocities
                    nan, nan, nan,   # attitudes
                    nan, nan, nan,   # gyros
                    nan, nan, nan, nan,  # motors
    ...
    def _send_motor_health(self, motor_num, health_val):
        ...
        try:
            ...
            param = Parameter()
            param.name = f'cf1.params.powerDist.m{motor_num}Health'
            param.value.type = ParameterType.PARAMETER_DOUBLE
            param.value.double_value = float(health_val)
            req.parameters.append(param)
            self.param_client.call_async(req)
        except Exception:
            pass  # Node is shutting down, param service already gone
